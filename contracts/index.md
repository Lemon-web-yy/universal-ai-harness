...
> **定位：** 跨模块契约档案（L1 发布点）的权威索引。**收尾归档前必须核对本索引与实体文件一致**（`python tools/audit_harness.py`）。

## 档案列表

| 编号 | 标题 | 日期 | 摘要 |
|------|------|------|------|
| （空 — 第一份契约用 `contracts/_template.md` 起草，编号从 001 开始） | | | |

## 写入纪律

1. 新契约编号 = 当前最大编号 + 1（三位数字递增）
2. 文件名 `NNN_<kebab-case-english-name>.md`，起草前先读 `contracts/_template.md`
